...
instance (KnownNat t) => Model (ANat t) where
  envType _ =
    return . TypeCon . TypVar . fromIntegral . anatVal $ (undefined :: ANat t)
...
useCT
  :: Typeable a
  => Maybe (ConTree String HTypeRef)
  -> proxy a
  -> State Env (Type (TypeRef QualName))
...
addCT_
  ...
  => Bool
  -> proxy a
  -> State Env (Maybe (ConTree String HTypeRef))
  -> State Env (Type (TypeRef QualName))
...
instance (Datatype d, GModel a, GModel b) => GModel (M1 D d (a :+: b) ) where
  ...
  gcontree x =
    ConTree <$> gcontree (unL . unM1 $ x) <*> gcontree (unR . unM1 $ x)
  gtype  = notThere
  gtypeN = notThere
...
instance (Selector c,GModel a) => GModel (M1 G.S c a) where
  ...
  gtype    = notThere
  gtypeN ~s@(M1 x) =
    (\t -> [let n = selName s in if null n then Left t else Right (n, t)])
      <$> gtype x
